...
export class DetailsComponent implements OnInit {
  ...
  createHourlyChart() {
    ...
    let data = this.hourlyCandle?.t.map((val, index) => {
      return [val, this.hourlyCandle?.c[index]];
    });
  ...
  fetchAll(ticker: string) {
    this.backendHelper.getQuote(ticker).subscribe(
      (values) => {
        ...
        // this.backendHelper.setMarketStatus(this.marketOpen);

        // let fromDate: number;
        // let toDate: number;
        // const marketCloseTime = 13 * 3600;
        // if (this.marketOpen) {
        //   toDate = values.t;
        //   fromDate = values.t - 86400;
        // } else {
        //   const currentTime = Math.floor(Date.now() / 1000);
        //   if (currentTime < marketCloseTime) {
        //     toDate = values.t - 86400; 
        ...
        if (this.marketOpen) {
          // Market is open, show stock price variation from yesterday to today
          const yesterday = new Date(this.currentTime);
          yesterday.setDate(yesterday.getDate() - 1); // Get yesterday's date
          from = yesterday.toISOString().split('T')[0]; // Format as YYYY-MM-DD
          to = new Date(this.currentTime).toISOString().split('T')[0]; // Format as YYYY-MM-DD
      } else {
          // Market is closed, show stock price variation from one day before closing to the closing time
          const oneDayBeforeClosing = new Date(this.closedTime);
          oneDayBeforeClosing.setDate(oneDayBeforeClosing.getDate() - 1); // Get one day before closing
          from = oneDayBeforeClosing.toISOString().split('T')[0]; // Format as YYYY-MM-DD
          to = new Date(this.closedTime).toISOString().split('T')[0]; // Format as YYYY-MM-DD
      }
   
        this.backendHelper.getHourlyCandle(ticker,from,to).subscribe(
          (values) => {
    ...
    this.backendHelper.getCandle(ticker,
   ...
   this.backendHelper.getSocial(ticker).subscribe(
      (values: SocialDAO) => {
        this.social = values;
            var insiderData=values;
         this.tMSPR += insiderData.totalMSPR;
            this.pMSPR += insiderData.positiveMSPR;
            this.nMSPR += insiderData.negativeMSPR;
            this.tChange += insiderData.totalChange;
            this.pChange += insiderData.positiveChange;
            this.nChange += insiderData.negativeChange;
      }
    );
